...
if [ ${stage} -le 2 ]; then
    ...
    data2json.sh --feat ${feat_tr_dir}/feats.scp --bpecode ${bpemodel}.model \
	 --adv ${adv} --spkid ${spkid} data/${train_set} ${dict} > ${feat_tr_dir}/data_${bpemode}${nbpe}.json
    # Split training data because this has all the speakers for adversarial training
    splitjson_spk.py ${feat_tr_dir}/data_${bpemode}${nbpe}.json
...
if [ -z ${tag} ]; then
    expdir=exp/${train_set}_${backend}_${etype}_e${elayers}_subsample${subsample}_unit${eunits}_proj${eprojs}_d${dlayers}_unit${dunits}_${atype}_aconvc${aconv_chans}_aconvf${aconv_filts}_mtlalpha${mtlalpha}_${opt}_sampprob${samp_prob}_bs${batchsize}_mli${maxlen_in}_mlo${maxlen_out}_adv${adv}
    if ${do_delta}; then
